...
import { Upload, Search, RefreshCw, FileSpreadsheet, Calculator, CheckCircle2, PlayCircle } from "lucide-react";
...
export default function TicketsPage() {
  ...
  const [dealsModal,   setDealsModal]   = useState<{ ticket: UploadedTicket; deals: DealMatchSummary[] } | null>(null);
  ...
  async function runCalculation(id: number) {
    ...
    try {
      const { data } = await api.patch<RunCalcResult>(`/tickets/uploads/${id}/run-calculation`);
  ...
  async function openDealsModal(ticket: UploadedTicket) {
    ...
    try {
      const { data } = await api.get<DealMatchSummary[]>(`/tickets/uploads/${ticket.id}/matched-deals`);
  ...
  async function runAllCalculation() {
    ...
    try {
      if (selectedIds.size > 0) {
        let processed = 0, matched = 0, unmatched = 0, errors = 0;
        for (const id of selectedIds) {
          try {
            const { data } = await api.patch<RunCalcResult>(`/tickets/uploads/${id}/run-calculation`);
            processed++;
            if (data.matched) matched++; else unmatched++;
            setTickets(prev => prev.map(t =>
              t.id === id ? { ...t, matched_deal_id: data.matched_deal_id, matched_deal_type: data.matched_deal_type, matched_deal_name: data.matched_deal_name, calculated_incentive: data.calculated_incentive } : t
            ));
          } catch { errors++; }
        }
        ...
        const { data } = await api.patch<BatchRunCalcResult>("/tickets/uploads/run-all-calculation");
        setBatchResult(data);
        await fetchTickets();
      }
    } catch {
  ...
  return (
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 uppercase tracking-wide">Tickets</h1>
          <p className="text-xs text-gray-500 mt-0.5">Supplier statement tickets uploaded from XLS</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={someSelected ? `Run calc for ${selectedIds.size} selected ticket(s)` : "Run calc for all tickets"}
            ...
            {batchRunning ? "Running…" : someSelected ? `Run Calc (${selectedIds.size})` : "Run All Calc"}
          </button>
          <Link href="/tickets/upload"
      ...
      {batchResult && (
        <div className="flex items-center gap-4 px-4 py-2 bg-amber-50 border border-amber-200 rounded-lg text-[11px] text-gray-600">
          <span>Processed: <strong className="text-gray-900">{batchResult.processed}</strong></span>
          <span className="text-green-700">Matched: <strong>{batchResult.matched}</strong></span>
          <span className="text-gray-500">Unmatched: <strong>{batchResult.unmatched}</strong></span>
          {batchResult.errors > 0 && <span className="text-red-600">Errors: <strong>{batchResult.errors}</strong></span>}
          <button onClick={() => setBatchResult(null)} className="ml-auto text-gray-400 hover:text-gray-600 text-[10px]">✕ dismiss</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { label: "Total Tickets",   value: tickets.length,             color: "bg-blue-50 text-blue-700 border-blue-200"     },
          { label: "Batches",         value: batches,                    color: "bg-indigo-50 text-indigo-700 border-indigo-200" },
          { label: "Total Sell Fare", value: `₹ ${fmt(totalFare)}`,      color: "bg-green-50 text-green-700 border-green-200"   },
          { label: "Net Amount",      value: `₹ ${fmt(totalNetAmt)}`,    color: "bg-purple-50 text-purple-700 border-purple-200" },
        ].map(s => (
          ...
          <table className="table-auto border-collapse">
            <thead>
              <tr style={{ background: "#1e3a5f" }}>
                {/* checkbox */}
                <th className="px-3 py-2.5 text-center whitespace-nowrap sticky left-0" style={{ background: "#1e3a5f" }}>
                  <input
                    type="checkbox"
                ...
                <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">CC</th>
                <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">Acc Code</th>
                {/* calculated incentive */}
                <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-amber-300 uppercase tracking-wider whitespace-nowrap">Calc. Incentive</th>
                {/* delta comm */}
                <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-orange-300 uppercase tracking-wider whitespace-nowrap">Delta Comm</th>
                {/* deal match */}
                <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">Matched Deal</th>
                {/* action */}
                <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap sticky right-0"
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={TOTAL_COLS} className="px-4 py-12 text-center text-xs text-gray-400">
                ...
                <tr>
                  <td colSpan={TOTAL_COLS} className="px-4 py-10 text-center text-xs text-red-400">{apiError}</td>
                </tr>
              ) : paginated.length === 0 ? (
                <tr>
                  <td colSpan={TOTAL_COLS} className="px-4 py-14 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs text-gray-400 font-medium">
                        {search ? "No tickets match your search." : "No tickets uploaded yet."}
                      </p>
                      {!search && (
                        <Link href="/tickets/upload"
                          className="flex items-center gap-1.5 bg-[#1e3a5f] text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-[#16304f]">
                ...
                return (
                  <tr key={t.id}
                    ...
                    {/* checkbox */}
                    <td className={`px-3 py-2 text-center whitespace-nowrap sticky left-0 ${isSelected ? "bg-amber-50/40" : rowBg}`}>
                      <input
                        type="checkbox"
                        checked={isSelected}
                    ...
                    {TEXT_HEADERS.map(h => {
                      ...
                      return (
                        <td key={h.key} className="px-3 py-2 text-[11px] text-gray-600 whitespace-nowrap">
                          {txt(val)}
                        </td>
                      );
                    })}
                    ...
                    {/* CC */}
                    <td className="px-3 py-2 text-[11px] text-gray-600 whitespace-nowrap">{txt(t.cc)}</td>
                    {/* Acc Code */}
                    <td className="px-3 py-2 text-[11px] text-gray-600 whitespace-nowrap">{txt(t.acc_code)}</td>

                    {/* Calculated Incentive */}
                    <td className="px-3 py-2 text-right text-[11px] whitespace-nowrap font-semibold">
                      {t.calculated_incentive != null ? (
                        <span className="text-amber-700">₹ {fmt(t.calculated_incentive)}</span>
                      ) : (
                    ...
                    <td className="px-3 py-2 text-right text-[11px] whitespace-nowrap font-semibold">
                      {t.comm_sell != null && t.calculated_incentive != null ? (() => {
                        const delta = (t.comm_sell ?? 0) - (t.calculated_incentive ?? 0);
                        return (
                          <span className={delta >= 0 ? "text-orange-600" : "text-red-600"}>
                            ₹ {fmt(delta)}
                          </span>
                        );
                      })() : <span className="text-gray-300">—</span>}
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">
                      {t.matched_deal_name ? (
                        ...
                        >
                          ...
                          {t.matched_deal_type && (
                            <span className={`self-start px-1.5 py-0 rounded text-[9px] font-bold uppercase ${
                    ...
                    {/* Run Calculation button — sticky right */}
                    <td className={`px-3 py-2 text-center whitespace-nowrap sticky right-0 ${isSelected ? "bg-amber-50/40" : rowBg}`}>
                      <button
                        onClick={() => runCalculation(t.id)}
                        disabled={isCalcLoading}
                        title="Match ticket against approved deals"
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[10px] font-semibold transition-colors disabled:opacity-50
                          bg-white border-[#1e3a5f] text-[#1e3a5f] hover:bg-[#1e3a5f] hover:text-white">
                        {isCalcLoading
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-start justify-between px-5 py-4 border-b border-gray-100">
            <div>
              <h2 className="text-sm font-bold text-gray-900">Matching Deals</h2>
              <p className="text-[11px] text-gray-400 mt-0.5">
                Ticket {dealsModal.ticket.ticket_number ?? dealsModal.ticket.booking_ref} · {dealsModal.deals.length} deal{dealsModal.deals.length !== 1 ? "s" : ""} found
              </p>
            </div>
            <button onClick={() => setDealsModal(null)} className="text-gray-400 hover:text-gray-700 text-lg leading-none">×</button>
          </div>
          ...
          <div className="overflow-y-auto flex-1 px-5 py-4">
            {dealsLoading ? (
              ...
              <p className="text-xs text-gray-400 text-center py-8">No matching deals found for this ticket.</p>
            ) : (
              <div className="space-y-2.5">
                {dealsModal.deals.map((d) => (
                  <div key={`${d.deal_type}-${d.deal_id}`}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`text-xs font-bold tabular-nums whitespace-nowrap ${d.is_best ? "text-green-700" : "text-gray-700"}`}>
                        {d.calculated_incentive != null ? `₹ ${fmt(d.calculated_incentive)}` : "—"}
                      </span>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-3 text-[10px] text-gray-500">
                      {d.valid_from && d.valid_to && (
                        <span>{d.valid_from.slice(0, 10)} → {d.valid_to.slice(0, 10)}</span>
                      )}
                      {d.deal_maker_name && <span>Maker: {d.deal_maker_name}</span>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="px-5 py-3 border-t border-gray-100 flex justify-end">
            <button onClick={() => setDealsModal(null)}
