...
import { categoryMarkupLabel, corporateTypeLabel, partyAddress, partyName, type Party as Corporate } from "@/lib/party";
...
const TABS = ["Corporate Details", "Sold Tickets", "Billing Info"] as const;
type Tab = (typeof TABS)[number];

const money = (n: number | null | undefined) =>
  n == null ? "—" : `₹${Number(n).toLocaleString("en-IN", { maximumFractionDigits: 2 })}`;

function markupLabel(c: Corporate): string {
 ...
 *  only applies it. */
function rowCalc(
  t: SoldTicket, additionalStr: string, discountStr: string, billingType: Corporate["billing_type"],
) {
...
export default function CorporateDetailPage() {
  ...
  const fetchCorporate = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get<Corporate>(`/corporates/${corporateId}`);
      setCorporate(data);
    } catch (e: unknown) {
      const status = (e as { response?: { status?: number } })?.response?.status;
      setError(status === 404 ? "Corporate not found." : "Failed to load corporate.");
    } finally {
  ...
  const fetchBillings = useCallback(async () => {
    ...
    try {
      const { data } = await api.get<BillingListItem[]>(`/corporates/${corporateId}/billings`);
      setBillings(data);
    } catch {
      /* ignore */
    } finally {
  ...
  useEffect(() => {
    fetchCorporate();
    fetchBillings();
  }, [fetchCorporate, fetchBillings]);

  /**
   * Load this corporate's tickets — its employees', matched through Employee Master.
   * Twin of customers/[id]/page.tsx — keep the two in step.
   *
  ...
  const summary = useMemo(() => {
    ...
    let base = 0,
      ...
      total = 0;
    for (const t of rows) {
      const c = rowCalc(t, additional[t.id] ?? "", discounts[t.id] ?? "", corporate?.billing_type ?? null);
      base += c.base;
      markup += c.custMarkup;
      addl += c.addl;
      gst += c.gst;
  ...
  }, [soldTickets, additional, discounts, corporate]);
  ...
  const selectedSummary = useMemo(() => {
    ...
    let total = 0;
    for (const t of rows) total += rowCalc(t, additional[t.id] ?? "", discounts[t.id] ?? "", corporate?.billing_type ?? null).total;
    return { count: rows.length, total };
  }, [soldTickets, selected, additional, discounts, corporate]);
  ...
  const openView = async (b: BillingListItem) => {
    ...
    try {
      const { data } = await api.get<BillingDetail>(`/corporates/${corporateId}/billings/${b.id}`);
      setViewBilling(data);
    } catch {
  ...
  const saveEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      await api.patch(`/corporates/${corporateId}/billings/${editBilling.id}`, {
  ...
  const downloadPdf = async (id: number) => {
    setDownloadingId(id);
    try {
      const res = await api.get(`/corporates/${corporateId}/billings/${id}/pdf`, { responseType: "blob" });
      const url = window.URL.createObjectURL(res.data as Blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `corporate-billing-${id}.pdf`;
  ...
  const downloadTickets = async () => {
    ...
    try {
      const res = await api.get(`/corporates/${corporateId}/tickets-export`, {
        params: appliedFilters ?? {},
        responseType: "blob",
      });
      const now = new Date();
      const stamp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
      const slug = (corporate?.company ?? "").replace(/[^A-Za-z0-9]+/g, "-").replace(/^-|-$/g, "").toLowerCase()
  ...
  const deleteBilling = async (b: BillingListItem) => {
    ...
    try {
      await api.delete(`/corporates/${corporateId}/billings/${b.id}`);
      fetchBillings();
    } catch {
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          <button onClick={() => router.push("/corporates")} className="mt-1 p-1.5 rounded-lg border border-gray-200 hover:bg-gray-50">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex">
          {TABS.map((t) => (
            ...
            >
              {t === "Corporate Details" ? <Building className="w-3.5 h-3.5" /> : t === "Sold Tickets" ? <Ticket className="w-3.5 h-3.5" /> : <FileText className="w-3.5 h-3.5" />}
              {t}
      ...
      {loadingCorporate ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            ...
            <DetailRow label="Default Markup Type" value={corporate.markup_type ? corporate.markup_type.charAt(0).toUpperCase() + corporate.markup_type.slice(1) : "—"} />
            <DetailRow label="Category Markups" value={categoryMarkupLabel(corporate) || "—"} />
            <DetailRow label="Markup" value={markupLabel(corporate)} />
            <DetailRow
              label="Billing Type"
              value={corporate.billing_type ? corporate.billing_type.charAt(0).toUpperCase() + corporate.billing_type.slice(1) : "—"}
            />
            <DetailRow label="Status" value={corporate.is_active ? "Active" : "Inactive"} />
          </div>
        </div>
      ) : tab === "Sold Tickets" ? (
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-100">
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    ...
                    {corporate.markup_type ? ` (${corporate.markup_type})` : ""}
                    {categoryMarkupLabel(corporate) ? ` · by category: ${categoryMarkupLabel(corporate)}` : ""} · Billing:{" "}
                    {corporate.billing_type
                    ...
                    <tbody>
                      {soldTickets.length === 0 ? (
                        ...
                        soldTickets.map((t, idx) => {
                          const c = rowCalc(t, additional[t.id] ?? "", discounts[t.id] ?? "", corporate.billing_type);
