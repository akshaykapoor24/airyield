...
import { Plus, Search, Edit2, Trash2, RefreshCw, Upload, Building, ChevronRight, AlertTriangle, X } from "lucide-react";
import api from "@/lib/api";
import Pagination from "@/components/ui/Pagination";
import CorporateModal, { type Corporate } from "@/components/corporates/CorporateModal";
import UploadCorporatesModal from "@/components/corporates/UploadCorporatesModal";

const PAGE_SIZE = 25;

function markupTypeLabel(c: Corporate): string {
  if (!c.markup_type) return "—";
  return c.markup_type === "percentage" ? "Percentage" : "Fixed";
}

function markupValueLabel(c: Corporate): string {
  if (c.markup_value == null || !c.markup_type) return "—";
  return c.markup_type === "percentage" ? `${c.markup_value}%` : `₹${c.markup_value}`;
}

function billingLabel(c: Corporate): string {
...
export default function CorporatesPage() {
  ...
  const fetchCorporates = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get<Corporate[]>("/corporates/", { params: { limit: 500 } });
      setCorporates(data);
    } catch {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/corporates/${deleteTarget.id}`);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-0.5">Corporates</p>
          <h1 className="text-xl font-bold text-gray-900">Corporate Billing</h1>
          <p className="text-xs text-gray-500 mt-0.5">{corporates.length} corporates</p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Plus className="w-3.5 h-3.5" /> Add Corporate
          </button>
        </div>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={11} className="px-4 py-12 text-center text-xs text-gray-400">
                    <RefreshCw className="w-4 h-4 animate-spin mx-auto mb-2 text-gray-300" /> Loading corporates…
                  </td>
                </tr>
              ) : pageItems.length === 0 ? (
                <tr>
                  <td colSpan={11} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm font-medium text-gray-600">No corporates yet</p>
                      <p className="text-xs text-gray-400 mt-1 mb-4">Add a corporate manually or import from Excel.</p>
                      <div className="flex gap-2">
                ...
                pageItems.map((c, idx) => (
                  <tr
                    key={c.id}
                    onClick={() => router.push(`/corporates/${c.id}`)}
      ...
      {showAdd && <CorporateModal onClose={() => setShowAdd(false)} onSaved={fetchCorporates} />}
      {showUpload && <UploadCorporatesModal onClose={() => setShowUpload(false)} onSaved={fetchCorporates} />}
      {editTarget && <CorporateModal corporate={editTarget} onClose={() => setEditTarget(null)} onSaved={fetchCorporates} />}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-gray-900">Delete Corporate</h2>
              </div>
