...
// Customer Statement upload wizard — B2B only. Mirrors the Internal Statement upload
// (Upload → Map Columns → Preview → Done), but the Statement Details form picks an
// Agency from the Customer Directory plus (optionally) its Entities and Login IDs.
// Reuses the same extraction service; writes to the separate customer-statements tables.
...
import {
  ...
  Building2, Calendar, FileText, KeyRound,
} from "lucide-react";
...
type LoginOpt  = { id: number; login_id: string; airline_name: string | null; entity_id: number | null; entity_name: string | null };
...
const OUR_FIELDS: FieldDef[] = [
  ...
  // ── B2B ──────────────────────────────────────────────────────────────────
  { key: "booking_ref",        label: "Booking Ref",         required: false },
...
// ── Agency custom dropdown (Customer Directory agencies) ───────────────────
function AgencyDropdown({ agencyId, agencyName, options, onPick, touched, fieldCls }: {
  agencyId: number | "";
  agencyName: string;
  options: AgencyOpt[];
  ...
  const filtered = options.filter(a => a.name.toLowerCase().includes(query.toLowerCase()));
    ...
    <div>
      ...
      <div className="relative" ref={ref}>
        <button
          type="button"
          onClick={() => setOpen(o => !o)}
          className={`${fieldCls(agencyId === "" ? "" : agencyName)} w-full text-left flex items-center justify-between pr-8`}
        >
          <span className={agencyName ? "text-gray-800" : "text-gray-400"}>
            ...
            <ul className="max-h-48 overflow-y-auto py-1">
              {filtered.length === 0 ? (
                ...
                <li key={a.id}>
                  <button
                    type="button"
                    onClick={() => { onPick(a.id, a.name); setOpen(false); setQuery(""); }}
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-blue-50 transition-colors ${
                      a.id === agencyId ? "bg-blue-50 text-blue-700 font-semibold" : "text-gray-700"
                    }`}
                  >
                    {a.name}
                  </button>
...
// ── Multi-select checkbox list (entities / login IDs) ──────────────────────
function CheckList({ items, selected, onToggle, empty }: {
  ...
  if (items.length === 0) return <p className="text-[11px] text-gray-400 italic px-1 py-2">{empty}</p>;
  return (
    <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-50">
      {items.map(it => (
        <label key={it.id} className="flex items-center gap-2 px-2.5 py-1.5 cursor-pointer hover:bg-blue-50/40">
          <input type="checkbox" checked={selected.has(it.id)} onChange={() => onToggle(it.id)} className="accent-blue-500 cursor-pointer shrink-0" />
          <span className="text-[11px] text-gray-700 truncate">{it.label}</span>
          {it.sub && <span className="text-[10px] text-gray-400 ml-auto shrink-0">{it.sub}</span>}
        </label>
      ))}
    </div>
  );
...
function StatementFormPanel({
  agencyId, agencyName, agencyOptions, onPickAgency,
  entities, selectedEntities, onToggleEntity,
  loginIds, selectedLogins, onToggleLogin,
  loadingDir,
  validFrom, setValidFrom, validTo, setValidTo,
  touched, isComplete,
}: {
  agencyId: number | ""; agencyName: string; agencyOptions: AgencyOpt[];
  onPickAgency: (id: number, name: string) => void;
  entities: EntityOpt[]; selectedEntities: Set<number>; onToggleEntity: (id: number) => void;
  loginIds: LoginOpt[]; selectedLogins: Set<number>; onToggleLogin: (id: number) => void;
  loadingDir: boolean;
  validFrom: string; setValidFrom: (v: string) => void;
  validTo: string; setValidTo: (v: string) => void;
  touched: boolean; isComplete: boolean;
}) {
  const fieldCls = (val: string) =>
    `border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 ${
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden h-fit sticky top-4">
      ...
      <div className="px-5 py-4 border-b border-gray-100" style={{background:"#1e3a5f"}}>
        ...
        <p className="text-xs text-white/60 mt-1">B2B customer statement — pick the agency, then its entities / login IDs</p>
      </div>

      <div className="px-5 py-5 space-y-4">
        {/* Agency */}
        <AgencyDropdown
          agencyId={agencyId} agencyName={agencyName} options={agencyOptions}
          onPick={onPickAgency} touched={touched} fieldCls={(v) => `w-full ${fieldCls(v)}`}
        />
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
            <Building2 className="w-3.5 h-3.5 inline mr-1" />
            Entities <span className="text-gray-400 normal-case font-normal">(optional)</span>
          </label>
          {agencyId === "" ? (
            <p className="text-[11px] text-gray-400 italic px-1 py-2">Select an agency first.</p>
          ) : loadingDir ? (
            <p className="text-[11px] text-gray-400 px-1 py-2"><RefreshCw className="w-3 h-3 inline animate-spin mr-1" />Loading…</p>
          ) : (
            <CheckList
              items={entities.map(e => ({ id: e.id, label: e.name, sub: e.code }))}
              selected={selectedEntities} onToggle={onToggleEntity}
              empty="No entities for this agency."
            />
          )}
        </div>
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
            ...
            Login IDs <span className="text-gray-400 normal-case font-normal">(optional)</span>
          </label>
          {agencyId === "" ? (
            <p className="text-[11px] text-gray-400 italic px-1 py-2">Select an agency first.</p>
          ) : loadingDir ? (
            ...
            <CheckList
              items={loginIds.map(l => ({
                id: l.id,
                label: l.login_id + (l.airline_name ? ` · ${l.airline_name}` : ""),
                sub: l.entity_name ?? undefined,
              }))}
              selected={selectedLogins} onToggle={onToggleLogin}
              empty="No login IDs for this agency."
            />
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className={`w-full ${fieldCls(validFrom)}`}
          />
          {touched && !validFrom && (
        ...
        <div>
          ...
          <input
            ...
            onClick={e => { try { (e.target as HTMLInputElement).showPicker(); } catch {} }}
            className={dateError ? "w-full border border-red-300 bg-red-50 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" : `w-full ${fieldCls(validTo)}`}
          />
          {touched && !validTo && (
        ...
        <div className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium ${
          ...
          {isComplete ? "Statement details complete" : "Select agency & valid dates"}
        </div>
      </div>
    </div>
  );
...
// ── component ──────────────────────────────────────────────────────────────
export default function CustomerStatementUploadPage() {
  ...
  useEffect(() => {
    api.get<AgencyOpt[]>("/agencies/?limit=1000")
      .then(r => setAgencyOptions(r.data.map(a => ({ id: a.id, name: a.name }))))
      .catch(() => {});
  }, []);

  const pickAgency = useCallback(async (id: number, name: string) => {
    setAgencyId(id); setAgencyName(name);
    setSelectedEntities(new Set()); setSelectedLogins(new Set());
    setEntities([]); setLoginIds([]);
    setLoadingDir(true);
    try {
      const [ent, log] = await Promise.all([
        api.get<EntityOpt[]>("/agency-entities/", { params: { agency_id: id, limit: 1000 } }),
        api.get<LoginOpt[]>("/agency-login-ids/", { params: { agency_id: id, limit: 1000 } }),
      ]);
      setEntities(ent.data);
      setLoginIds(log.data);
    } catch {
      setEntities([]); setLoginIds([]);
    } finally {
      setLoadingDir(false);
  ...
  const toggleLogin = useCallback((id: number) =>
    setSelectedLogins(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; }), []);

  const isStatementComplete =
    agencyId !== "" && validFrom !== "" && validTo !== "" && validTo >= validFrom;
  ...
  const handleDownloadTemplate = async () => {
    setDownloading(true);
    try {
      const resp = await api.get(`/customer-statements/template/download`, { responseType: "blob" });
      const url  = URL.createObjectURL(resp.data as Blob);
      const a    = downloadLinkRef.current!;
      a.href     = url;
      a.download = "customer_statement_template.xlsx";
      a.click();
  ...
  const onDrop = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      const { data } = await api.post<ExtractionPreview>("/customer-statements/upload/extract", form, {
  ...
  const confirmUpload = async () => {
    ...
    try {
      const { data } = await api.post<{ batch_id: string; created_count: number }>(
        "/customer-statements/upload/confirm",
        {
          file_name:  preview.file_name,
          rows,
          agency_id:  typeof agencyId === "number" ? agencyId : null,
          agency:     agencyName,
          entities:   entities.filter(e => selectedEntities.has(e.id)).map(e => ({ id: e.id, name: e.name, code: e.code })),
          login_ids:  loginIds.filter(l => selectedLogins.has(l.id)).map(l => ({ id: l.id, login_id: l.login_id, airline_name: l.airline_name, entity_id: l.entity_id, entity_name: l.entity_name })),
      ...
      if (storedFile && data.batch_id) {
        try {
          ...
          await api.post(`/customer-statements/statements/${data.batch_id}/file`, fd, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        } catch (uploadErr) {
          console.error("[GCS] Customer statement file upload FAILED:", uploadErr);
        }
      }
      setStep("done");
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
      setError(msg ?? "Failed to save statement.");
      setStep("preview");
    }
  };
  ...
  const reset = () => {
    ...
    setAgencyId(""); setAgencyName(""); setEntities([]); setLoginIds([]);
    setSelectedEntities(new Set()); setSelectedLogins(new Set());
    setValidFrom(""); setValidTo(""); setFormTouched(false);
  };

  const isDone = step === "done";
  const selCount = `${selectedEntities.size} ent · ${selectedLogins.size} logins`;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 uppercase tracking-wide">Upload Customer Statement</h1>
            <p className="text-xs text-gray-500 mt-0.5">Create a B2B statement and import the XLS/XLSX</p>
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto text-center space-y-5 py-10">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-700 mt-1">
              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold mr-2 bg-purple-100 text-purple-700">B2B</span>
              {agencyName}
            </p>
            <p className="text-xs text-gray-500 mt-0.5">{validFrom} – {validTo} · {selCount}</p>
      ...
      {step === "drop" && (
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6 items-start">
          {/* LEFT: Statement form */}
          <StatementFormPanel
            agencyId={agencyId} agencyName={agencyName} agencyOptions={agencyOptions} onPickAgency={pickAgency}
            entities={entities} selectedEntities={selectedEntities} onToggleEntity={toggleEntity}
            loginIds={loginIds} selectedLogins={selectedLogins} onToggleLogin={toggleLogin}
            loadingDir={loadingDir}
            validFrom={validFrom} setValidFrom={setValidFrom}
            validTo={validTo} setValidTo={setValidTo}
            touched={formTouched} isComplete={isStatementComplete}
          />
          {/* RIGHT: dropzone */}
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-5 py-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-blue-800 mb-0.5">Download the statement template</p>
      ...
      {step === "mapping" && preview && (
        <div className="space-y-4">
          ...
          <div className="bg-[#1e3a5f]/5 border border-[#1e3a5f]/20 rounded-xl px-5 py-3 flex items-center gap-4 flex-wrap">
            ...
            <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide bg-purple-100 text-purple-700">B2B</span>
            <span className="text-xs font-semibold text-[#1e3a5f]">{agencyName || <span className="text-red-400 italic">Agency missing</span>}</span>
            ...
            <span className="text-gray-300 text-xs">·</span>
            <span className="text-xs text-gray-500">{selCount}</span>
            {!isStatementComplete && (
              <button
