...
// Customer Statements repository — B2B statements uploaded for the sub-agencies we
// onboard in the Customer Directory. Mirrors the Internal Statement repo (/tickets),
// B2B-only, with an Entities/Login-IDs count from the picked directory selection.
...
import {
  ...
  Building2, Calendar, Hash, AlertCircle, Search, X, KeyRound,
} from "lucide-react";
import api from "@/lib/api";

type Snap = { id: number; name?: string; code?: string; login_id?: string; entity_name?: string };
...
export default function CustomerStatementsPage() {
  ...
  const fetchStatements = async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get<CustomerStatement[]>("/customer-statements/statements");
      setStatements(data);
    } catch {
      setError("Failed to load customer statements. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const hasFilter = filterName || filterAgency || filterFrom || filterTo;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 uppercase tracking-wide">Customer Statements</h1>
          <p className="text-xs text-gray-500 mt-0.5">B2B statements uploaded for the agencies in your Customer Directory</p>
        </div>
      ...
      {!loading && !error && statements.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm font-medium text-gray-600">No customer statements yet</p>
          <p className="text-xs text-gray-400 mt-1 mb-5">Upload your first B2B customer statement to get started</p>
          <Link
            href="/customers/statements/upload"
            ...
            <table className="w-full">
              <thead>
                <tr style={{background:"#1e3a5f"}}>
                  ...
                  <th className="px-4 py-2 text-left text-[11px] font-semibold text-white/80 uppercase tracking-wide">
                    <span className="flex items-center gap-1"><KeyRound className="w-3.5 h-3.5" /> Entities / Logins</span>
                  </th>
                  <th className="px-4 py-2 text-left text-[11px] font-semibold text-white/80 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 ? (
                  ...
                  <tr
                    key={stmt.batch_id}
                    onClick={() => router.push(`/customers/statements/${stmt.batch_id}`)}
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <p className="text-sm font-semibold text-gray-800 group-hover:text-[#1e3a5f]">
                          {stmt.statement_name ?? `B2B · ${stmt.agency}`}
                        </p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-2">
                      <span className="text-xs text-gray-600">
                        {(stmt.entities?.length ?? 0)} ent · {(stmt.login_ids?.length ?? 0)} logins
                      </span>
                    </td>
