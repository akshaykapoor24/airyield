...
export default function UploadCorporatesModal({
  onClose,
  onSaved,
}: {
  onClose: () => void;
  ...
  const handleTemplateDownload = async () => {
    setError("");
    try {
      const res = await api.get("/corporates/template", { responseType: "blob" });
      const url = window.URL.createObjectURL(res.data as Blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "corporate_template.xlsx";
  ...
  const handleUpload = async () => {
    ...
    fd.append("file", file);
    try {
      const { data } = await api.post<BulkResult>("/corporates/bulk-upload", fd, {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-gray-900">Import Corporates</h2>
          </div>
