...
function EntityModal({ entity, onClose, onSaved }: { entity: EntityRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit && entity) await api.patch(`/user-entities/${entity.id}`, body);
      else await api.post("/user-entities/", body);
      onSaved();
  ...
  return (
    <ModalShell title={isEdit ? "Edit Entity" : "Add Entity"} onClose={onClose}>
      ...
      {(isEdit || tab === "manual") ? (
        ...
        <UploadBox resource="user-entities" templateName="user_entity_template.xlsx" columns="NAME, CODE, ADDRESS, STATE, CITY, ACTIVE" onDone={onSaved} />
      )}
    </ModalShell>
  );
}
